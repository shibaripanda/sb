...
const varanti = async (status) => {
    if(status[status.length - 1].indexOf('❌') == -1){
        return false
    }
    return 'hide'
}
...
export const arhivMenu = async (user) => {
    ...
    if(user.ordersArhiv.length !== 0){
        ...
        keyboard = Markup.inlineKeyboard([
            [Markup.button.callback('Гарантия (заказать звонок)', `varanti|${user.ordersArhiv[user.orderIndex][0].globalNumber}`, await varanti(user.ordersArhiv[user.orderIndex][0].status))],
